#!/bin/bash
# Run 5 iterations of each performance test configuration
...
echo "Running 5 iterations of each configuration"
...
run_test() {
    ...
    while [ $retry -lt $max_retries ]; do
        if [ $retry -gt 0 ]; then
            echo "[$test_name - Run $run_number/5] Retry attempt $retry/$((max_retries-1))..."
            sleep 15  # Extra cooldown before retry
        else
            echo "[$test_name - Run $run_number/5] Starting..."
        fi
        ...
        if npx wdio "$config"; then
            ...
            if [ "$moved" = true ]; then
                echo "[$test_name - Run $run_number/5] ✓ Completed"
                echo ""
                ...
                echo "[$test_name - Run $run_number/5] ✗ Result file not found"
            fi
        else
            echo "[$test_name - Run $run_number/5] ✗ Test execution failed"
        fi

        retry=$((retry + 1))

        if [ $retry -eq $max_retries ]; then
            echo "[$test_name - Run $run_number/5] ✗✗✗ FAILED after $max_retries attempts"
            exit 1
        fi
    done
}

# Run No Agent tests (5 iterations)
echo "=========================================="
...
echo ""
echo "Run aggregate script to analyze results:"
echo "  node aggregate-results.js"
echo "=========================================="
