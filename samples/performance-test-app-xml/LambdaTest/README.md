# Session Replay Performance Testing - XML Layouts

This directory contains automated performance tests for measuring New Relic Session Replay overhead on an Android application using traditional XML layouts.
...
```bash
cd /path/to/performance-test-app-xml
...
This test app uses **traditional XML layouts** with Android Views:
- **MaterialButton** - for buttons and interactions
- **RecyclerView** - for infinite scroll and image gallery
- **ScrollView** - for scrollable content
- **LinearLayout/ConstraintLayout** - for layout structure

All UI elements have `contentDescription` attributes for Appium accessibility:
```xml
<com.google.android.material.button.MaterialButton
    android:id="@+id/btnInfiniteScroll"
    android:text="Infinite Scroll"
    android:contentDescription="Infinite Scroll" />
```
...
- Ensure MaterialButtons have `contentDescription` attributes
- Use accessibility ID selectors (`~"Button Name"`) instead of text selectors
...
- XML layouts typically use less memory than Jetpack Compose (~40-50 MB less)

## Related Documentation

- **Jetpack Compose Test:** See `../performance-test-app/LambdaTest/` for Jetpack Compose tests
