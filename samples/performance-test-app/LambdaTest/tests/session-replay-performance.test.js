...
describe("Session Replay Performance Impact - Android", () => {
  it("Should perform intensive UI operations and collect resource metrics", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      try {
        // Find and click "Infinite Scroll" card
        const infiniteScrollCard = await $('android=new UiSelector().text("Infinite Scroll")');
    ...
    try {
      ...
      const imageGalleryCard = await $('android=new UiSelector().text("Image Gallery")');
    ...
    try {
      // Click on UI Elements card
      const uiElementsCard = await $('android=new UiSelector().text("UI Elements")');
    ...
    try {
      // Click on Network Test card
      const networkTestCard = await $('android=new UiSelector().text("Network Test")');
    ...
    try {
      // Click on Navigation Test card
      const navigationCard = await $('android=new UiSelector().text("Navigation Test")');
