...
/**
 * Compatibility coverage for the AGP 9.4.x support ceiling (NR-620918).
 *
 * Pins AGP 9.4.0 / Gradle 9.6.1 (minifyEnabled=true, compiler=r8) and asserts the AGP90Adapter
 ...
 * It also guards {@link BuildHelper#maxSupportedAGPVersion} against regressing below 9.4.0,
 * which would re-introduce the "may not be compatible" warning for this toolchain.
 */
@Requires({ jvm.isJava17Compatible() })
class PluginAGP94CompatSpec extends PluginSpec {

    static final agpVersion = "9.4.0"
    ...
    def setupSpec() {
        given: "create the build runner pinned to the AGP 9.4.x toolchain"
    ...
    def "build the test app on AGP 9.4.x"() {
    ...
    def "verify AGP 9.4.x is within the supported ceiling"() {
