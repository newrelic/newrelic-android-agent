...
/**
 * Compatibility coverage for the AGP 9.2.x support ceiling (NR-586734).
 *
 * Pins the validated toolchain from NR-584136 (AGP 9.2.1 / Gradle 9.6.1, minifyEnabled=true,
 * compiler=r8) and asserts the AGP90Adapter path instruments and re-emits classes correctly:
 ...
 * It also guards {@link BuildHelper#maxSupportedAGPVersion} against regressing below 9.2.1,
 * which would re-introduce the "may not be compatible" warning for this toolchain.
 */
@Requires({ jvm.isJava17Compatible() })
class PluginAGP92CompatSpec extends PluginSpec {

    static final agpVersion = "9.2.1"
    static final gradleVersion = "9.6.1"
    ...
    def setupSpec() {
        given: "create the build runner pinned to the AGP 9.2.x toolchain"
    ...
    def "build the test app on AGP 9.2.x"() {
    ...
    def "verify AGP 9.2.x is within the supported ceiling"() {
        expect: "no unsupported-AGP warning is emitted for the validated toolchain"
