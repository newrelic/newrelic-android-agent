...
public final class BytecodeBuilder {
    ...
    public BytecodeBuilder loadInvocationDispatcher() {
        ...
        // RewriterAgent.INVOCATION_DISPATCHER_FIELD_NAME is a static field so we can just load null for the get invocation
        mv.visitInsn(Opcodes.ACONST_NULL);
    ...
    public BytecodeBuilder printToInfoLogFromBytecode(final String message) {
        ...
        mv.visitLdcInsn(RewriterAgent.PRINT_TO_INFO_LOG);
        mv.visitInsn(Opcodes.ACONST_NULL);
     ...
     * method.
     *
     * @param popReturnOffStack if true, the return value will be popped off the stack.
     * @return
     */
