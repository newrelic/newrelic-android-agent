...
public class AgentMethodDelegateClassVisitor extends ClassVisitor {
    ...
    private static final class DelegateMethodVisitor extends GeneratorAdapter {
        ...
        public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean isInterface) {
            ...
            if (!tryShadowMethods(opcode, owner, name, desc)) {
            ...
            for (final ClassMethod replacementMethod : replacementMethods) {
                ...
                if (isSuperCallInOverride) {
                    log.debug(MessageFormat.format(
                            "[{0}] skipping call site replacement for super call in overridden method: {1}:{2}",
                ...
                if (opcode == Opcodes.INVOKESPECIAL && name.equals("<init>")) {
                    ...
                    // instanceof type check and only wraps the appropriate types. Always do the instance check
                    // because it covers the invocation target being null. Unfortunately, to do a type
                    // check we have to pop all the arguments off of the stack
        ...
        private boolean tryShadowMethods(final int opcode, final String owner, final String name, final String desc) {
            final Collection<ClassMethod> shadowClassMethods = context.getShadowMethods(owner, name, desc);

            if (shadowClassMethods.isEmpty()) {
                return false;
            }

            ClassMethod originalClassMethod = new ClassMethod(owner, name, desc);
            Method originalMethod = new Method(name, desc);

            for (final ClassMethod shadowClassMethod : shadowClassMethods) {
                //
                // INVOKESPECIAL is used to invoke a method directly on a super class (e.g.
                // when you do something like `super.execute(request)` in the derived class.).
                //
                ...
                if (isSuperCallInOverride) {
                    log.debug(MessageFormat.format(
                            "[{0}] skipping shadow method replacement for super call in overridden method: {1}:{2}",
                            context.getClassName().replaceAll("/", "."), this.name, this.desc));
                    return false;
                }
                ...
                if (opcode == Opcodes.INVOKESPECIAL && name.equals("<init>")) {
                    ...
                    // be responsible for creating the object.  To discard the original object, we first store all the args
                    // in locals.  Next, we pop the two objects off the stack (note the dup call) and then reload the args.
                    // Finally, we'll call the static replacement method and its return value will be on terop of the stack.
                    ...
                    log.debug(MessageFormat.format("[{0}] tracing shadow constructor call to {1} - {2}", context.getFriendlyClassName(), originalClassMethod.toString(), owner));
                    ...
                    // Finally, call the replacement method
                    super.visitMethodInsn(Opcodes.INVOKESTATIC, shadowClassMethod.getClassName(), shadowClassMethod.getMethodName(), shadowClassMethod.getMethodDesc(), false);
                    ...
                    log.debug(MessageFormat.format("[{0}] shadowing static call to {1} with {2}",
                            context.getClassName().replaceAll("/", "."), originalClassMethod.toString(), shadowClassMethod.toString()));

                    super.visitMethodInsn(Opcodes.INVOKESTATIC, originalClassMethod.getClassName(), originalClassMethod.getMethodName(), originalClassMethod.getMethodDesc(), false);

                    // super.visitMethodInsn(Opcodes.INVOKESTATIC, shadowClassMethod.getClassName(), shadowClassMethod.getMethodName(), shadowClassMethod.getMethodDesc(), false);

                } else {
                    Method newMethod = new Method(shadowClassMethod.getMethodName(), shadowClassMethod.getMethodDesc());

                    // Always do the instance check because it covers the invocation target being null

                    log.debug(MessageFormat.format("[{0}] shadowing call to {1} with {2} (with instance check)",
                            context.getClassName().replaceAll("/", "."), originalClassMethod.toString(), shadowClassMethod.toString()));

                    // We assume that if a method signature exactly matches the one we're interested in then the invocation
                    // target is of the correct type, but that may not be the case, so the code we inject does a runtime
                    // instanceof type check and only wraps the appropriate types. Unfortunately, to do a
                    // type check we have to pop all the arguments off of the stack
                    ...
                    super.visitMethodInsn(Opcodes.INVOKESTATIC,
                            shadowClassMethod.getClassName(), shadowClassMethod.getMethodName(), shadowClassMethod.getMethodDesc(), false);
