...
### 4. Coverage-gap fix — `WebViewMethodClassVisitor.java`

Today, `onPageFinished` is only instrumented when the app's class **directly** extends `android.webkit.WebViewClient` (anchored regex `^android/webkit/WebViewClient$`). A `WebViewClient` subclassed through an intermediate class (e.g. a library's own base client) is invisible to this visitor, silently skipping both the existing `PageFinished` metric and the new detection script.

Broaden the match to catch any depth of `WebViewClient` subclassing, following the same non-anchored pattern already used for `WebView` itself in `WEBVIEW_CLASSES`. This directly serves this ticket's goal — an undercounted `onPageFinished` hook undercounts browser-agent detection by the same amount — so it's in scope here rather than deferred to a follow-up.
